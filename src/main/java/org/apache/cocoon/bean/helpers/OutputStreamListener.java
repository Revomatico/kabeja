 ...
 * @version CVS $Id: OutputStreamListener.java,v 1.2 2005/02/15 13:06:59 simijhag Exp $
 */
public class OutputStreamListener implements BeanListener {
    ...
    public void pageGenerated(
        ...
        if (linksInPage == -1) {
            ...
            this.print(pad(12,
                    "* [" + pagesComplete + "/" + pagesRemaining + "] ")
                + pad(10, "[" + newLinksInPage + "/" + linksInPage + "] ")
                + pad(7, time + "s ") + pad(7, size) + " " + sourceURI);
        }
    }
    ...
    public void complete() {
        ...
        this.print("Total time: " + (duration / 60000) + " minutes "
            + ((duration % 60000) / 1000) + " seconds, " + " Site size: "
            + df.format(this.siteSize) + " Site pages: " + this.sitePages);
        this.close();
    }
    ...
    private void outputBrokenLinksAsText() {
        ...
        try {
            writer = new PrintWriter(new FileWriter(new File(reportFile)), true);
